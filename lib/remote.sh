...
remote_download()
{
  ...
  for ((c = ${retries}; c >= 0; c--)); do
    dryrun "rsync -e \"ssh -o PasswordAuthentication=no -o PubkeyAuthentication=yes -o LogLevel=ERROR $*\" --delete-before -avzx '${target}:${sourcefile}' '${destfile}' > /dev/null"
    if test $? -eq 0; then
      return 0
...
remote_upload()
{
  ...
  local c
  for ((c = ${retries}; c >= 0; c--)); do
    dryrun "rsync -e \"ssh -o PasswordAuthentication=no -o PubkeyAuthentication=yes -o LogLevel=ERROR $*\" --delete-before -avzx '${sourcefile}' '${target}:${destfile}' > /dev/null"
    if test $? -eq 0; then
      return 0
    elif test $c -gt 0; then
...
remote_exec()
{
  ...
  shift 2
  dryrun "ssh -o PasswordAuthentication=no -o PubkeyAuthentication=yes -o LogLevel=ERROR $* \"${target}\" \"${cmd}\""
  return $?
}
...
remote_exec_async()
{
  ...
  dryrun "ssh -n -o PasswordAuthentication=no -o PubkeyAuthentication=yes -o LogLevel=ERROR $* ${target} -- \"exec 1>${stdoutfile}; exec 2>${stderrfile}; ${cmd}; echo EXIT CODE: \\\$?\" &"
