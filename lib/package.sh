...
#    local installdir="`dirname ${installdir} | sed -e 's:/bin::'`"
	...
	if test `echo ${host} | grep -c mingw` -eq 0; then
	    ...
	    local basedir="`dirname ${local_builds}`"
	    # --platform "Linux-x86_64"
	    ${local_snapshots}/infrastructure/installjammer-1.2.15/installjammer --output-dir ${local_snapshots}/ -DVersion ${version}-${target} --platform "Windows" -DBaseDir ${basedir} --build ${abe_path}/config/jammer/Linaro-${target}.mpi
	fi
    fi
