...
if test x"${fileserver}" != x; then
	...
	# compare_dg_tests.pl returns:
	# 2 in case of regressions or no PASS was found
	# 1 if only improvements were found
	# For the time being we only care about regressions
	if test $? -eq 2; then
	    ret=1
	    diff2_ret=1
	else
	...
	echo "Comparison through report.sh returned $diff1_ret"
	echo "Comparison through compare_dg_tests.pl returned $diff2_ret"
    done
    rm -fr ${tmp}
